...
argparser = argparse.ArgumentParser(
    description='Bring up iter8 analytics service.')
argparser.add_argument('-p', '--promconfig', metavar="<path/to/promconfig.json>",
                       help='prometheus configuration file', required=True)
argparser.add_argument('-m', '--metricsconfig', metavar="<path/to/promconfig.json>",
                       help='metrics configuration file', required=True)
...
class CanaryCheckAndIncrement(Resource):
    ...
    def post(self):
        ...
        self.metric_factory = Iter8MetricFactory(
            prom_config["metric_backend_url"])
